...
class TemporalFusionTransformer(object):
  ...
  def _batch_sampled_data(self, data, max_samples):
    ...
    for identifier, df in data.groupby(id_col):
      ...
      num_entries = len(df)
      if num_entries >= self.time_steps or num_entries < self.time_steps:
        valid_sampling_locations += [
  ...
  def _batch_data(self, data):
    ...
    def _batch_single_entity(input_data):
      ...
      elif time_steps < lags:
         return np.stack([x[i:time_steps - (time_steps - 1) + i, :]
                          for i in range(time_steps)], axis=1)
         #print(outstack)
         #out = np.vstack(
         #(outstack[0, time_steps-1], x[time_steps-1]))
         #for i in range(50):
         #out = np.vstack(
         #(out[0], x[time_steps-1]))
         #return out
      else:
        return None
  ...
  def _build_base_graph(self):
    ...
    unknown_inputs = None
#TODO This part selects the future and historical inputs. We need to remove it.
    #TODO Healthy case and different targets. Then setting this to none.
  ...
  def predict(self, df, return_targets=False):
    ...
    #data = self._batch_sampled_data(df, 1)
    ...
    def format_outputs(prediction):
      ...
      print("PREDICTION BEFORE FORMATTING")
      print(prediction)
      ...
      #print("time: ", time, " identifier: ", identifier)
    ...
    # Extract inputs for different entries
  ...
  def dataFormatLive(self, df):
    #print("DF: ", df)
    dataout = np.array([[0.0, 0.0, 0.0]])
    dtemp = np.array([0.0, 0.0, 0.0])
    for i in range(len(df['time_from_start'])):
        dtemp[0] = df['time_from_start'][i]
        dtemp[1] = df['health_index'][i]
        dtemp[2] = df['categorical_id'][i]
        if len(dataout) == 1:
            dataout = dtemp
        else:
            dataout = np.vstack((dataout, dtemp))
    #print("dataout: ", dataout, "length: ", len(dataout))
    if len(dataout) > 150:
        dataout = dataout[len(dataout)-151:-1]
        #print("WINDOWED")
    dataout = np.array([dataout], dtype=object)
    #print("dataout: ", dataout, "length: ", len(dataout[0]))
    return dataout

  def predictLive(self, df, return_targets=False):
    """Computes predictions for a given input dataset.
      ...
      Returns:
        Input dataframe or tuple of (input dataframe, aligned output dataframe).
     """
    #TODO match the data format without data batches and use predict from keras.
    #TODO create sliding window (queue) for history and samples.

    data = self.dataFormatLive(df)
    ...
    def format_outputs(prediction):
        ...
        cols = list(flat_prediction.columns)
        flat_prediction['forecast_time'] = time[len(time)- 51]
        flat_prediction['identifier'] = identifier[:]
