...
class A2D2Formatter(GenericDataFormatter):
  ...
  def set_scalers(self, df):
    ...
    for identifier, sliced in df.groupby(id_column):
      ...
      if len(sliced) >= self._time_steps or len(sliced) < self._time_steps:
        data = sliced[real_inputs].values
        targets = sliced[[target_column]].values
        self._real_scalers[identifier] = sklearn.preprocessing.StandardScaler().fit(data)

        #TODO change the targets to history.
  ...
  def transform_inputs(self, df):
    ...
    #output[real_inputs] = self._real_scalers[identifiers].transform(df[real_inputs].values)
  ...
  def format_predictions(self, predictions):
    ...
    df_list = []
    #identifier, sliced = predictions.groupby("identifier")
    ...
    for identifier, sliced in predictions.groupby('identifier'):
        ...
        for col in column_names:
          if col not in {'forecast_time', 'identifier'}:
            #cut = sloced[col]
            sliced[col] = target_scaler.inverse_transform(sliced[col])
