...
class STMFModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if len(self.opt.gpu_ids) > 0:
            self.state = Variable(torch.zeros(self.opt.batch_size, 256, int(
                math.floor(self.opt.image_size_H/8)), int(self.opt.image_size_W/8)).cuda(), requires_grad=False)
        else:
            self.state = Variable(torch.zeros(self.opt.batch_size, 256, int(
                self.opt.image_size_H/8), int(self.opt.image_size_W/8)), requires_grad=False)

        self.inputs = []
        self.feature_map = []
        for i in range(self.opt.K + self.opt.T):
            self.inputs.append(self.Tensor(
                self.opt.batch_size, self.opt.c_dim, self.opt.image_size_H, self.opt.image_size_W))
        ...
        if self.opt.is_train:
            ...
            if opt.adversarial:
                ...
                #self.discriminator = STMF_network.define_discriminator(opt.image_size, opt.c_dim, self.K, self.T, #self.opt.K / self.opt.T
                #                                                          opt.df_dim, gpu_ids=self.gpu_ids) #opt.gf_dim
                self.discriminator = STMF_network.define_discriminator(opt.image_size, opt.c_dim, self.opt.K, self.opt.T,  # self.opt.K / self.opt.T
                                                                       opt.gf_dim, gpu_ids=self.gpu_ids)  # opt.gf_dim
            ...
            for optimizer in self.optimizers:
                self.schedulers.append(
                    STMF_network.get_scheduler(optimizer, opt))

    def forward(self):
        if not self.updateG and self.opt.is_train:
            with torch.no_grad():
                print("NO GRAD!")
                self.pred, _ = self.generator.forward(self.inputs, self.state)
        if self.opt.create_dataset:
            with torch.no_grad():
                print("NO TRAINING!")
                self.pred, feat = self.generator.forward(self.inputs, self.state)
                for i in feat:
                    temp = i.cpu().detach().numpy()
                    temp = torch.squeeze(temp)
                    self.feature_map.append(temp)
        else:
            self.pred, _ = self.generator.forward(self.inputs, self.state)
        #self.set_input(self.pred)
    ...
    def get_current_visuals(self):
        ...
        for i in self.pred:
            pred.append(i.cpu())
        return {'seq_batch': input, 'pred': pred}

    def backward_D(self):
        # fake
        input_fake = torch.cat(self.inputs[:self.K] + self.pred, dim=1)
        input_fake_ = Variable(input_fake.data)
        h_sigmoid, h = self.discriminator.forward(
            input_fake_, self.opt.batch_size)
        ...
        h_sigmoid_, h_ = self.discriminator.forward(
            input_real_, self.opt.batch_size)
        if len(self.gpu_ids) > 0:
    ...
    def backward_G(self):
        ...
        targets = util.inverse_transform(
            torch.cat(self.inputs[self.opt.K:], dim=0))
        ...
        if self.opt.adversarial:
            ...
            h_sigmoid, h = self.discriminator.forward(
                input_fake, self.opt.batch_size)
            if len(self.gpu_ids) > 0:
            ...
            self.loss_G += self.opt.beta1 * self.L_GAN
        self.loss_G.backward()

    def optimize_parameters(self):
        self.forward()
        self.opt.D_G_switch = 'adaptive'
        self.opt.margin = 0.1
        if not self.opt.adversarial:
            ...
            if self.opt.D_G_switch == 'adaptive':
                ...
                raise NotImplementedError(
                    'switch method [%s] is not implemented' % self.opt.D_G_switch)

    def get_current_errors(self):
        if not self.opt.no_adversarial:
            return OrderedDict([('G_GAN', self.L_GAN.detach().item()),
                                ('G_Lp', self.Lp.detach().item()),
                                ('G_gdl', self.gdl.detach().item()),
                                ('G_loss', self.loss_G.detach().item()),
                                ('D_real', self.loss_d_real.detach().item()),
                                ('D_fake', self.loss_d_fake.detach().item())
                                ])
        else:
            return OrderedDict([('G_Lp', self.Lp.detach().item()),
                                ('G_gdl', self.gdl.detach().item()),
                                ('G_loss', self.loss_G.detach().item())
                                ])
    ...
    def set_inputs(self, input):
        ...
        if len(self.gpu_ids) > 0:
            for i in range(self.opt.K + self.opt.T):
                self.inputs.append(
                    Variable(input[:, :, :, :, i].cuda(), volatile=f_volatile))
        else:
            for i in range(self.opt.K + self.opt.T):
                self.inputs.append(
                    Variable(input[:, :, :, :, i], volatile=f_volatile))
    ...
    def save(self, label, epoch):
        ...
        if len(self.gpu_ids) and torch.cuda.is_available():
            ...
            self.generator.encoder.cuda(device=self.gpu_ids[1])
            if self.opt.adversarial:
