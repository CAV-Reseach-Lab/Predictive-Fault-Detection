...
def main():
    ...
    with Parallel(n_jobs=opt.batch_size) as parallel:
            ...
            for _, batchidx in mini_batches:
                ...
                if len(batchidx) == opt.batch_size:
                    ...
                    if opt.create_dataset:
                        if trainfiles[batchidx].find('anomaly') != -1:
                            label = 1
                        else:
                            label = 0
                        labels.append(label)
                        labels = np.stack(labels, dim=0)
        ...
        print("end training")
        if opt.create_dataset:
            with open('train.pt', 'wb+') as outfile:
                labels = np.squeeze(labels)
                data = [model.feature_map, labels]
                outDataset = dataset(data)
                pickle.dump(outDataset, outfile)
