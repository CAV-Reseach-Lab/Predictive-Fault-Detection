...
class Generator(nn.Module):
    def __init__(self, opt):
        ...
        self.image_size = [opt.image_size_H, opt.image_size_W]

        self.encoder = define_encoder(opt)
        #num_features = 128
        self.convLstm_cell = define_convLstm_cell(
            feature_size=3, num_features=opt.gf_dim * 8, gpu_ids=self.gpu_ids)
        self.decoder = define_decoder(
            c_dim=opt.c_dim, gf_dim=opt.gf_dim, gpu_ids=self.gpu_ids)

    def forwardK(self, inputs, state):
        for k in range(self.K):

            #print("forward function, k=", k)
            #print("inputs:", len(inputs))

            #print("inputs[K]:", np.shape(inputs[k]))
            if len(self.gpu_ids) > 0:
                h_encoder = self.encoder.forward(
                    inputs[k].cuda(self.gpu_ids[1]))
                h_dyn, state = self.convLstm_cell.forward(
                    h_encoder.cuda(self.gpu_ids[0]), state)
            else:
                h_encoder = self.encoder.forward(
                    inputs[k])
                h_dyn, state = self.convLstm_cell.forward(
    ...
    def forwardT(self, inputs, state):
        pred = []
        #torch.cuda.empty_cache()
        for t in range(self.T):
            ...
            x_hat = self.decoder.forward(inputs)
            #if torch.cuda.is_available():
                #x_hat = x_hat.cpu()
            #xt = x_hat
            pred.append(x_hat.view(self.batch_size, self.c_dim,
                                   self.image_size[0], self.image_size[1]))
            #torch.cuda.empty_cache()
        return pred

    def forward(self, inputs, state):
        h_dyn, state = self.forwardK(inputs, state)
        #torch.cuda.empty_cache()
        pred = self.forwardT(h_dyn, state)
...
def define_encoder(opt, init_type='STMF'):
    ...
    encoder = Encoder(opt)
    if use_gpu:
        encoder.cuda(device=opt.gpu_ids[1])
...
class Encoder(nn.Module):
    def __init__(self, opt):
        ...
        self.conv1 = nn.Conv2d(opt.c_dim, nOut_conv1, kernel_size=(
            3, 3), bias=True, padding=(1, 1), stride=(1, 1),)
        self.dense1 = self._make_dense(
            nIn_dense1, opt.growthRate, nDenseBlocks, opt.bottleneck)
        self.trans1 = Transition(nIn_trans1, nOut_trans1)
        self.dense2 = self._make_dense(
            nIn_dense2, opt.growthRate, nDenseBlocks, opt.bottleneck)
        self.trans2 = Transition(nIn_trans2, nOut_trans2)
        self.dense3 = self._make_dense(
            nIn_dense3, opt.growthRate, nDenseBlocks, opt.bottleneck)
    ...
    def forward(self, x):
        ...
        out = self.trans2(out)
        #print(out.size(), out_dwt1.size())
        #out_dwt1 = out_dwt1.reshape([8, 40, 75, 120])
...
class Dwtconv(nn.Module):
    def __init__(self, outC1, outC2, outC3):
        ...
        #raise Exception("These hard-coded /'4/' are interfering!!")
        nIn_conv1 = 4 * 3
        self.conv1_1 = nn.Conv2d(
            nIn_conv1, outChannel_conv1, kernel_size=3, padding=1)
        self.bn1_1 = nn.BatchNorm2d(outChannel_conv1)
        self.relu1_1 = nn.ReLU()
        self.conv1_2 = nn.Conv2d(
            outChannel_conv1, outC1, kernel_size=3, padding=1)
        ...
        self.conv2_1 = nn.Conv2d(
            nIn_conv2, outChannel_conv2, kernel_size=3, padding=1)
        self.bn2_1 = nn.BatchNorm2d(outChannel_conv2)
        self.relu2_1 = nn.ReLU()
        self.conv2_2 = nn.Conv2d(
            outChannel_conv2, outC2, kernel_size=3, padding=1)
        ...
        self.conv3_1 = nn.Conv2d(
            nIn_conv3, outChannel_conv3, kernel_size=3, padding=1)
        self.bn3_1 = nn.BatchNorm2d(outChannel_conv3)
        self.relu3_1 = nn.ReLU()
        self.conv3_2 = nn.Conv2d(
            outChannel_conv3, outC3, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        #print("dwt1_1_H [0]:", np.shape(dwt1_1_h[0]))

        out1 = torch.cat(
            (dwt1_1_l, dwt1_1_h[0][:, :, 0], dwt1_1_h[0][:, :, 1], dwt1_1_h[0][:, :, 2]), dim=1)
        #print("dwt1_1:", np.shape(dwt1_1))
        #print(np.shape(dwt1_1_h[1]))
        ...
        out2 = torch.cat(
            (dwt2_1_l, dwt2_1_h[0][:, :, 0], dwt2_1_h[0][:, :, 1], dwt2_1_h[0][:, :, 2]), dim=1)
        ...
        out3 = torch.cat(
            (dwt3_1_l, dwt3_1_h[0][:, :, 0], dwt3_1_h[0][:, :, 1], dwt3_1_h[0][:, :, 2]), dim=1)
...
class ConvLstmCell(nn.Module):
    ...
    def forward(self, input, state):
        ...
        new_c = c * F.sigmoid(f + self.forget_bias) + \
...
def define_discriminator(img_size, c_dim, in_num, out_num, df_dim, init_type="STMF", gpu_ids=[]):
    ...
    discriminator = Discriminator(
        img_size, c_dim, in_num, out_num, df_dim, gpu_ids)
...
class Discriminator(nn.Module):
    def __init__(self, img_size, c_dim, in_num, out_num, df_dim, gpu_ids):
        ...
        conv0 = nn.Conv2d(c_dim * (in_num + out_num),
                          df_dim, 4, stride=2, padding=1)
        ...
        D = [conv0, lrelu0, conv1, bn1, lrelu1,
             conv2, bn2, lrelu2, conv3, bn3, lrelu3]
    ...
    def forward(self, input, batch_size):
        if len(self.gpu_ids) > 0 and isinstance(input.data, torch.cuda.FloatTensor):
            ...
            h_sigmoid = nn.parallel.data_parallel(
                self.sigmoid, h, self.gpu_ids)
            return h_sigmoid, h
        else:
